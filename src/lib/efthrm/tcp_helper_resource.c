                        ...
                        void* vi_state, tcp_helper_cluster_t* thc)
{
  ...
  OO_STACK_FOR_EACH_INTF_I(ni, intf_i) {
    ...
    ni->nic_hw[intf_i].vi.efct_rxqs.q[0].superbufs = NULL;
#if CI_CFG_PIO
  ...
  OO_STACK_FOR_EACH_INTF_I(ni, intf_i) {
    ...
    nsn->vi_channel_or_irq = nic->flags & NIC_FLAG_EVQ_IRQ ?
                                        (ci_uint16)efrm_vi_get_irq(vi_rs) :
                                        (ci_uint16)efrm_vi_get_channel(vi_rs);
                            ...
                            tcp_helper_resource_t* trs)
{
  ...
  if( NI_OPTS(ni).prealloc_packets && trs->trs_efct_alloc ) {
    int n_rxqs = 0;
    OO_STACK_FOR_EACH_INTF_I(ni, intf_i) {
      struct efhw_nic* nic =
    ...
    if( n_rxqs ) {
      ...
      rc = oo_hugetlb_pages_prealloc(trs->trs_efct_alloc, nr_pages);
      if( rc < 0 ) {
        if( rc == -ENOSPC )
                        ...
                        tcp_helper_resource_t** rs_out)
{
  ...
  rs->trs_efct_alloc = oo_hugetlb_allocator_create(alloc->in_efct_memfd);
  if( IS_ERR(rs->trs_efct_alloc) ) {
    long rc = PTR_ERR(rs->trs_efct_alloc);
    rs->trs_efct_alloc = NULL;
 ...
 fail_pktbuf_memfd:
  if( rs->trs_efct_alloc )
    oo_hugetlb_allocator_put(rs->trs_efct_alloc);
 fail_efct_memfd:
#if CI_CFG_NIC_RESET_SUPPORT
...
void tcp_helper_dtor(tcp_helper_resource_t* trs)
{
  ...
  if( trs->trs_efct_alloc )
    oo_hugetlb_allocator_put(trs->trs_efct_alloc);
