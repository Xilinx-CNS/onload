...
ef10_nic_check_supported_filters(struct efhw_nic *nic) {
   ...
   */
  if( _ef10_nic_check_supported_filter(out, num_matches, MC_FILTER_IP_LOCAL) )
    nic->flags |= NIC_FLAG_RX_FILTER_TYPE_IP_LOCAL;
  if( _ef10_nic_check_supported_filter(out, num_matches, MC_FILTER_IP_FULL) )
    nic->flags |= NIC_FLAG_RX_FILTER_TYPE_IP_FULL;
  if( _ef10_nic_check_supported_filter(out, num_matches,
                                       MC_FILTER_VLAN_IP_WILD) )
    nic->flags |= NIC_FLAG_VLAN_FILTERS;
  if( _ef10_nic_check_supported_filter(out, num_matches, MC_FILTER_ETH_LOCAL) )
    nic->flags |= NIC_FLAG_RX_FILTER_TYPE_ETH_LOCAL;
  if( _ef10_nic_check_supported_filter(out, num_matches,
                                       MC_FILTER_ETH_LOCAL_VLAN) )
    nic->flags |= NIC_FLAG_RX_FILTER_TYPE_ETH_LOCAL_VLAN;
  if( _ef10_nic_check_supported_filter(out, num_matches,
                                       MC_FILTER_IP_PROTOCOL) )
    nic->flags |= NIC_FLAG_RX_FILTER_IP4_PROTO;
  if( _ef10_nic_check_supported_filter(out, num_matches, MC_FILTER_ETHERTYPE) )
    nic->flags |= NIC_FLAG_RX_FILTER_ETHERTYPE;
  if( _ef10_nic_check_supported_filter(out, num_matches,
                                       MC_FILTER_MAC_IP4_PROTO) )
    nic->flags |= NIC_FLAG_RX_FILTER_MAC_IP4_PROTO;
  if( _ef10_nic_check_supported_filter(out, num_matches,
                                       MC_FILTER_UCAST_MISMATCH) )
    nic->flags |= NIC_FLAG_RX_FILTER_TYPE_UCAST_MISMATCH;
  if( _ef10_nic_check_supported_filter(out, num_matches,
                                       MC_FILTER_MCAST_MISMATCH) )
...
static int ef10_translate_dma_addrs(struct efhw_nic* nic,
                                    const dma_addr_t *src,
                                    dma_addr_t *dst, int n)
{
  /* ef10 NICs have 1:1 mappings */
