...
#define IP_PRM_(x) (((x)>>24)&0xFF), (((x)>>16)&0xFF), \
                   (((x)>> 8)&0xFF), (((x)>> 0)&0xFF)
#define IP_PRM(x) IP_PRM_(ntohl(x))
      ...
      for( i = 0; i < bind_retries; ++i ) {
        ...
        if( (c->hash_type & HASH_DST) ) {
          ...
          while( gen_hash(c->hash_type, &addr, raddr) != c->hash_id ) {
            ...
            if( verbose >= 2 )
              fprintf(stderr, "tproxy_preload: not binding to "IP_FMT":%d as will not "
                              "match RSS got %d vs wanted %d\n",
                              IP_PRM(addr.sin_addr.s_addr), ntohs(addr.sin_port),
        ...
        if( (ntohl(addr.sin_addr.s_addr) & 0xFF) == 0 || addr.sin_port == 0)
          continue;
        if( verbose )
          fprintf(stderr, "tproxy_preload: binding %d, to "IP_FMT":%d\n",
                  sockfd, IP_PRM(addr.sin_addr.s_addr), ntohs(addr.sin_port));
        ...
        if( errno == EINVAL ) {
          if( verbose ) {
            ...
            fprintf(stderr, "tproxy_preload: ERROR: already bound to "
                    IP_FMT":%d rc %d\n",
                    IP_PRM(addr.sin_addr.s_addr), ntohs(addr.sin_port), rc);
          }
...
static void parse_params(void)
{
  ...
  do {
      ...
      if( (rc = sscanf(curr, "connect_bind=%15[0-9.]-%15[0-9.]@hash=%3[a-z]:%d/%d",
                       addr_s, addr_e, hash_name,
                       &tconnect.hash_id, &tconnect.hash_size)) >= 1 &&
