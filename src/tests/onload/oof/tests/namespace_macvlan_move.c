...
{
  struct ooft_hwport* hw0 = ooft_alloc_hwport(cp, net_ns,
                                              OOFT_HWPORT_EF10_FF);
  struct ooft_hwport* hw1 = ooft_alloc_hwport(cp, net_ns,
                                              OOFT_HWPORT_EF10_FF);
...
int test_namespace_macvlan_move(void)
{
  ...
  struct ooft_ifindex* macvlan = ooft_alloc_ifindex(cp, ns1,
                                                    EFX_FILTER_VID_UNSPEC,
                                                    mac);
  ooft_add_hwport_to_ifindex(macvlan, hw, ns1);
