 ...
 */
static int efx_farch_rx_dc_size = 64;
 ...
 */
static int efx_farch_tx_dc_size = 16;


#ifdef EFX_NOT_UPSTREAM

module_param_named(rx_desc_cache_size, efx_farch_rx_dc_size, int, 0644);
MODULE_PARM_DESC(rx_desc_cache_size, "[SFC4000/SFC9000-family] Set RX descriptor cache size");

module_param_named(tx_desc_cache_size, efx_farch_tx_dc_size, int, 0644);
MODULE_PARM_DESC(tx_desc_cache_size, "[SFC4000/SFC9000-family] Set TX descriptor cache size");

static bool efx_farch_disable_port_on_mpe = false;
module_param_named(efx_disable_port_on_mpe, efx_farch_disable_port_on_mpe,
		   bool, 0644);
MODULE_PARM_DESC(efx_disable_port_on_mpe,
		 "[SFC4000/SFC9000-family] Disable interface on Memory Parity Error");
...
/* Maximum flush errors per rx queue */
#define EFX_MAX_RX_QUEUE_FLUSH  10
...
static inline void efx_write_buf_tbl(struct efx_nic *efx, efx_qword_t *value,
				     unsigned int index)
{
	efx_sram_writeq(efx, efx_mem(efx, efx->type->buf_tbl_base),
			value, index);
}
...
fail:
	netif_err(efx, hw, efx->net_dev,
		  ...
		  " at address %#x mask "EFX_OWORD_FMT"\n", EFX_OWORD_VAL(reg),
			 ...
			 int a, int b)
{
	...
	for (index = 0; index < table->rows; index += table->step) {
		...
		if (memcmp(&reg, &buf, sizeof(reg))) {
			netif_err(efx, hw, efx->net_dev,
				  "table test failed at address %#x"
				  " index %#x. wrote "EFX_OWORD_FMT
				  " read "EFX_OWORD_FMT"\n",
				  table->address, index,
...
*/
static inline bool efx_farch_may_push_tx_desc(struct efx_tx_queue *tx_queue,
...
void efx_farch_tx_write(struct efx_tx_queue *tx_queue)
{
	...
	if (efx_farch_may_push_tx_desc(tx_queue, old_write_count)) {
...
static inline void
efx_farch_build_rx_desc(struct efx_rx_queue *rx_queue, unsigned int index)
{
...
int efx_farch_rx_init(struct efx_rx_queue *rx_queue)
{
	...
	EFX_POPULATE_OWORD_10(rx_desc_ptr,
			      ...
			      /* For kernel-mode queues in Siena, the
			       * JUMBO flag enables scatter.
			       */
			      FRF_AZ_RX_DESCQ_JUMBO, efx->rx_scatter,
 ...
 */
static int efx_farch_flush_queues_wait(struct efx_nic *efx,
...
{
	unsigned long timeout = msecs_to_jiffies(EFX_MAX_FLUSH_TIME);
	struct efx_channel *channel;
	struct efx_rx_queue *rx_queue;
	struct efx_tx_queue *tx_queue;
	int rc = 0;
	...
	rc = efx_farch_flush_queues_wait(efx, timeout);
...
efx_farch_handle_tx_event(struct efx_channel *channel, efx_qword_t *event)
{
	...
	if (likely(EFX_QWORD_FIELD(*event, FSF_AZ_TX_EV_COMP))) {
		...
		/* TX_EV_PKT_ERR can be caused by a PCIe error, in
		 * which case one or more TX queues may have been
		 * disabled (SF bugs 10727, 11028).
		 */
				      ...
				      const efx_qword_t *event)
{
	...
	bool rx_ev_frm_trunc, rx_ev_tobe_disc = 0;
...
{
	struct efx_tx_queue *tx_queue;
	int qid = EFX_QWORD_FIELD(*event, FSF_AZ_DRIVER_EV_SUBDATA);

	tx_queue = efx_get_tx_queue_from_index(efx, qid);
	if (tx_queue)
...
efx_farch_handle_rx_flush_done(struct efx_nic *efx, efx_qword_t *event)
{
	...
	if (failed) {
		netif_info(efx, hw, efx->net_dev,
			   "RXQ %d flush retry\n", qid);
		if (++rx_queue->failed_flush_count < EFX_MAX_RX_QUEUE_FLUSH) {
			rx_queue->flush_pending = true;
			atomic_inc(&efx->rxq_flush_pending);
		}
	} else {
...
efx_farch_handle_driver_event(struct efx_channel *channel, efx_qword_t *event, int budget)
{
	...
	default:
		...
		{
			int spent = efx_dl_handle_event(&efx->dl_nic, event, budget);
			if (spent)
...
bool efx_farch_ev_mcdi_pending(struct efx_channel *channel)
{
	...
	for (;;) {
		...
		if (!efx_event_present(&event))
			return false;
		ev_code = EFX_QWORD_FIELD(event, FSF_AZ_EV_CODE);
		if (ev_code == FSE_CZ_EV_CODE_MCDI_EV)
			return true;
	}
}
...
#ifdef EFX_NOT_UPSTREAM
		if (mem_perr && efx_farch_disable_port_on_mpe)
			efx_schedule_reset(efx, RESET_TYPE_DISABLE);
		else
...
#ifdef EFX_NOT_UPSTREAM
/* Also:
 * res->buffer_table_min      buftbl entries for Onload
 * res->buffer_table_lim      end of buftbl entries for Onload
 *                            (usually same as efx->rx_dc_base)
 */
#endif
int efx_farch_dimension_resources(struct efx_nic *efx, unsigned int sram_lim_qw)
{
#ifdef EFX_NOT_UPSTREAM
#ifdef CONFIG_SFC_DRIVERLINK
	struct efx_dl_falcon_resources *res = &efx->farch_resources;
	unsigned entries_per_vnic, vnic_limit;
	unsigned int min_entries_per_vnic;
	unsigned buftbl_free;
#endif
#endif
	unsigned int n_rx_queues, n_tx_queues, n_xdp_queues;
	unsigned vi_dc_entries;
	unsigned vi_count, buftbl_min;

	efx->sram_lim_qw = sram_lim_qw;

	n_rx_queues = efx_rx_channels(efx);
	if (efx->extra_channel_type[EFX_EXTRA_CHANNEL_PTP])
		++n_rx_queues;
	n_tx_queues = efx_tx_channels(efx) * EFX_TXQ_TYPES;
	n_xdp_queues = efx->xdp_tx_queue_count;

	if (efx_farch_rx_dc_size & (efx_farch_rx_dc_size - 1) ||
	    efx_farch_rx_dc_size < 8 || efx_farch_rx_dc_size > 64) {
		netif_err(efx, drv, efx->net_dev,
			  "bad rx_desc_cache_size=%d\n", efx_farch_rx_dc_size);
		return -EINVAL;
	}
	efx->rx_dc_entries = efx_farch_rx_dc_size;

	if (efx_farch_tx_dc_size & (efx_farch_tx_dc_size - 1) ||
	    efx_farch_tx_dc_size < 8 || efx_farch_tx_dc_size > 32) {
		netif_err(efx, drv, efx->net_dev,
			  "bad tx_desc_cache_size=%d\n", efx_farch_tx_dc_size);
		return -EINVAL;
	}
	efx->tx_dc_entries = efx_farch_tx_dc_size;
	vi_dc_entries = efx->rx_dc_entries + efx->tx_dc_entries;

	/* Account for the buffer table entries backing the datapath channels
	 * and the descriptor caches for those channels.
	 */
	buftbl_min = ((n_rx_queues * EFX_MAX_DMAQ_SIZE +
		       n_tx_queues * EFX_MAX_DMAQ_SIZE +
		       n_xdp_queues * EFX_MAX_DMAQ_SIZE +
		       efx_channels(efx) * EFX_MAX_EVQ_SIZE)
		      * sizeof(efx_qword_t) / EFX_BUF_SIZE);
	vi_count = max(efx_channels(efx), efx_tx_channels(efx) * EFX_TXQ_TYPES);

#ifdef EFX_NOT_UPSTREAM
#ifdef CONFIG_SFC_DRIVERLINK
	if (vi_count < res->evq_timer_min)
	...
	/* num_vis takes precedence over VFs */
	if (efx_target_num_vis >= 0) {
		unsigned int vnic_count, vnic_limit;

		vnic_count = (unsigned int) efx_target_num_vis;
		vnic_limit = min(buftbl_free / min_entries_per_vnic,
				res->evq_timer_lim - vi_count);

		if (vnic_count > vnic_limit) {
			netif_err(efx, drv, efx->net_dev,
				  "Reducing VI count from %d to %d\n",
	...
	efx->tx_dc_base = sram_lim_qw - vi_count * efx->tx_dc_entries;
	efx->rx_dc_base = efx->tx_dc_base - vi_count * efx->rx_dc_entries;

#ifdef EFX_NOT_UPSTREAM
#ifdef CONFIG_SFC_DRIVERLINK
	res->buffer_table_min = buftbl_min;
	WARN_ON(efx->rx_dc_base < res->buffer_table_min);

	/* The buffer table extends up to RX_DC_BASE but the region
	 * usable by Onload may be smaller
	 */
	res->buffer_table_lim = min(res->buffer_table_lim, efx->rx_dc_base);
	if (res->buffer_table_lim < res->buffer_table_min)
		res->buffer_table_min = res->buffer_table_lim;

	res->evq_timer_min = max(res->evq_timer_min, efx_channels(efx));
	res->evq_int_min = max(res->evq_int_min, efx_channels(efx));
	res->rxq_min = max(res->rxq_min, n_rx_queues);
	res->rxq_lim = min(res->rxq_lim, vi_count);
	res->txq_min = max(res->txq_min, n_tx_queues);
...
struct efx_farch_filter_table {
	...
	unsigned int	step;		/* step between entries */
	unsigned int	used;		/* number currently used */
	unsigned long	*used_bitmap;
	struct efx_farch_filter_spec *spec;
	unsigned int	search_limit[EFX_FARCH_FILTER_TYPE_COUNT];
};
...
efx_farch_filter_spec_table_id(const struct efx_farch_filter_spec *spec)
{
	...
	BUILD_BUG_ON(EFX_FARCH_FILTER_TABLE_TX_IP !=
		     EFX_FARCH_FILTER_TABLE_RX_IP + 3);
	BUILD_BUG_ON(EFX_FARCH_FILTER_TABLE_TX_MAC !=
		     EFX_FARCH_FILTER_TABLE_RX_MAC + 3);
...
static void efx_farch_filter_push_rx_config(struct efx_nic *efx)
{
	...
	if (table->size) {
		 ...
		 * bit here.
		 */
		EFX_SET_OWORD_FIELD(filter_ctl, FRF_BZ_SCATTER_ENBL_NO_MATCH_Q,
				    efx->rx_scatter);
	}
...
static void efx_farch_filter_push_tx_limits(struct efx_nic *efx)
{
	...
	table = &state->table[EFX_FARCH_FILTER_TABLE_TX_IP];
	if (table->size) {
		EFX_SET_OWORD_FIELD(
				  ...
				  struct efx_farch_filter_spec *spec)
{
	...
	case EFX_FARCH_FILTER_TABLE_TX_IP: {
		bool is_udp = (spec->type == EFX_FARCH_FILTER_UDP_FULL ||
			       spec->type == EFX_FARCH_FILTER_UDP_WILD);
...
static const enum efx_farch_filter_table_id efx_farch_filter_range_table[] = {
	...
	EFX_FARCH_FILTER_TABLE_TX_IP,	/* TX match pri 0 */
	EFX_FARCH_FILTER_TABLE_TX_IP,
	EFX_FARCH_FILTER_TABLE_TX_MAC,
	EFX_FARCH_FILTER_TABLE_TX_MAC,	/* TX match pri 3 */
};
				   ...
				   unsigned int filter_idx)
{
	...
	if (unlikely(table->used == 0)) {
		...
		if (table->id >= EFX_FARCH_FILTER_TABLE_TX_IP)
			efx_farch_filter_push_tx_limits(efx);
		else
			efx_farch_filter_push_rx_config(efx);
	}
			      ...
			      u32 *rss_context, int rxq_i, int stack_id)
{
	...
	/* farch does not support custom RSS contexts */
	if (rss_context && *rss_context)
		return -EINVAL;
	...
	/* rss_context should be NULL for a non-RSS filter, non-NULL (and
	 * pointing to a 0, checked above) for an RSS filter.  We do not
	 * support changing the RSS flag of a filter on farch (it would cause
	 * the filter hash to change).
	 */
	if (!(spec->flags & EFX_FILTER_FLAG_RX_RSS) != !rss_context) {
		rc = -EINVAL;
		goto out_unlock;
	}
	spec->dmaq_id = rxq_i;
	if (table_id == EFX_FARCH_FILTER_TABLE_RX_DEF) {
		...
		efx_farch_filter_build(&filter, spec);
		efx_writeo(efx, &filter,
...
static void efx_debugfs_read_filter_table(struct seq_file *file,
					  struct efx_farch_filter_table *table)
{
	size_t i;
	...
	if (!table->used_bitmap || !table->spec) {
		seq_printf(file, "table id=%d, used bitmap=%p, spec=%p\n",
			   table->id, table->used_bitmap, table->spec);
		return;
	}
	seq_printf(file, "table id=%d, offset=%u, size=%u, step=%u, used=%u\n",
		   table->id, table->offset, table->size, table->step,
		   table->used);
	for (i = 0; i < table->size; ++i) {
		if (!test_bit(i, table->used_bitmap))
			continue;

		efx_farch_filter_to_gen_spec(&spec, &table->spec[i]);
		efx_debugfs_print_filter(filter, sizeof(filter), &spec);
		seq_printf(file, "%zu = %s\n", i, filter);
	}
	seq_printf(file, "search_limits =");
	for (i = 0; i < EFX_FARCH_FILTER_TYPE_COUNT; ++i)
		seq_printf(file, " %u", table->search_limit[i]);
	seq_printf(file, "\n");
}

/* dump filter state via debugfs */
static int efx_debugfs_read_filter_list(struct seq_file *file, void *data)
{
	struct efx_farch_filter_state *state = data;
	size_t i;

	if (!state)
		seq_printf(file, "filter state uninitialized\n");
	else
		for (i = 0; i < EFX_FARCH_FILTER_TABLE_COUNT; ++i)
			efx_debugfs_read_filter_table(file, &state->table[i]);
...
#ifdef EFX_NOT_UPSTREAM
#ifdef CONFIG_SFC_DRIVERLINK
	_EFX_PARAMETER(struct efx_farch_filter_state, kernel_blocked,
				     ...
				     unsigned int index)
{
	...
	WARN_ON(table->spec[index].priority != EFX_FILTER_PRI_HINT);

	if (table->spec[index].priority == EFX_FILTER_PRI_HINT) {
