...
down () {
  ...
  if /sbin/ifconfig "$ethif" >/dev/null 2>&1; then
    /sbin/ifconfig "$ethif" down
  else
...
donet () {
  for ethif in $(list_net_dev); do
    down $ethif
    if [ -f "/sys/bus/pci/drivers/$N_MOD/unbind" ]; then
      local device_link="$(readlink "/sys/class/net/$ethif/device")"
      local pci_dev="$(basename "$device_link")"
      echo -n "$pci_dev" > "/sys/bus/pci/drivers/$N_MOD/unbind"
    fi
  done

  for m in onload sfc_char sfc_resource sfc_affinity sfc_mtd sfc_control sfc_mdio_trace "$N_MOD"; do
