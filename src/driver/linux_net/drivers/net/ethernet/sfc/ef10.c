...
/* Non-X4 VFs use BAR 0/1 for memory */
static unsigned int efx_ef10_vf_mem_bar(struct efx_nic *efx)
{
	return 0;
}

/* X4 VFs use BAR 2/3 for memory */
static unsigned int efx_x4_vf_mem_bar(struct efx_nic *efx)
{
	return 2;
}
...
static int efx_ef10_init_datapath_caps(struct efx_nic *efx)
{
	...
	if (outlen >= MC_CMD_GET_CAPABILITIES_V2_OUT_LEN) {
		u16 port = MCDI_ARRAY_BYTE(outbuf,
					   ...
					   nic_data->pf_index);
		BUILD_BUG_ON(MC_CMD_GET_CAPABILITIES_V2_OUT_INCOMPATIBLE_ASSIGNMENT <
			     MC_CMD_GET_CAPABILITIES_V2_OUT_NUM_VIS_PER_PORT_NUM);
		if (port < MC_CMD_GET_CAPABILITIES_V2_OUT_NUM_VIS_PER_PORT_NUM)
...
static const struct efx_hw_stat_desc efx_ef10_stat_desc[EF10_STAT_COUNT] = {
	...
	X4_DMA_STAT(exp_ctpio_write_bytes),
	X4_DMA_STAT(exp_ctpio_packets),
	X4_DMA_STAT(exp_ctpio_bytes),
	X4_DMA_STAT(exp_ctpio_warm_packets),
	X4_DMA_STAT(exp_ctpio_warm_bytes),
	X4_DMA_STAT(exp_ctpio_underflow_packets),
	X4_DMA_STAT(exp_ctpio_underflow_bytes),
	X4_DMA_STAT(exp_ctpio_drain_packets),
	X4_DMA_STAT(exp_ctpio_drain_bytes),
	X4_DMA_STAT(exp_tx_merge_packets),
	X4_DMA_STAT(exp_tx_event_tlps),
	X4_DMA_STAT(exp_tx_event_bytes),
	X4_DMA_STAT(exp_rx_packets),
	X4_DMA_STAT(exp_rx_bytes),
	X4_DMA_STAT(exp_rx_bad_packets),
	X4_DMA_STAT(exp_rx_port_disabled_packets),
	X4_DMA_STAT(exp_rx_port_disabled_bytes),
	X4_DMA_STAT(exp_rx_vfifo_full_drop_packets),
	X4_DMA_STAT(exp_rx_vfifo_full_drop_bytes),
	X4_DMA_STAT(exp_rx_dp_q_disabled_packets),
	X4_DMA_STAT(exp_rx_dp_q_disabled_bytes),
	X4_DMA_STAT(exp_rx_nodesc_drop_packets),
	X4_DMA_STAT(exp_rx_nodesc_drop_bytes),
	X4_DMA_STAT(exp_rx_oversize_trunc_bytes),
	X4_DMA_STAT(exp_rx_data_tlps),
	X4_DMA_STAT(exp_rx_data_bytes),
	X4_DMA_STAT(exp_rx_meta_tlps),
...
#ifdef EFX_NOT_UPSTREAM
		count = efx_auxbus_send_poll_event(efx_nic_to_probe_data(efx),
						   channel->channel, event,
						   budget);
#if IS_MODULE(CONFIG_SFC_DRIVERLINK)
		/* Fall back to driverlink if there were no legacy Onload
		 * clients on the auxiliary bus.
		 */
		if (count == -ENODEV)
			count = efx_dl_handle_event(&efx->dl_nic, event,
						    budget);
#endif
		return count;
#endif
