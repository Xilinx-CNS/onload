...
int efx_mae_start_counters(struct efx_nic *efx, struct efx_rx_queue *rx_queue)
{
	...
	MCDI_SET_WORD(inbuf, MAE_COUNTERS_STREAM_START_V2_IN_QID, rx_queue->queue);
...
int efx_mae_stop_counters(struct efx_nic *efx, struct efx_rx_queue *rx_queue)
{
	...
	for (i = 0; (i < (outlen / 4)) && (i < EFX_TC_COUNTER_TYPE_MAX); i++) {
		efx->tc->flush_gen[i] = MCDI_ARRAY_DWORD(
						outbuf,
...
#else /* EFX_TC_OFFLOAD */
int efx_mae_get_tables(struct efx_nic *efx) { return 0; }
void efx_mae_free_tables(struct efx_nic *efx) {}
#endif
				 ...
				 struct netlink_ext_ack *extack)
{
	...
	/* Can't filter on ip_firstfrag either */
