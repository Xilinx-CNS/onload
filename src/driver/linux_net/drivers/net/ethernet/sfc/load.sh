...
donet () {
  ...
  # Bug 82108: Riverhead only comes up with local MAC addresses so filter out the Riverhead driver
  mac_local=("$(echo "$intf_by_mac" | grep -v sfc_ef100 | sed -n 's/^\(0[26ae][0-9a-fA-F:]*\) \(.*\) sfc.*/\1(\2)/p')")
  ...
  if [ $NO_IFUP -eq 0 ] ; then
      # bring up interfaces
      suffixindex=0
      houseeth=`ip route | grep default | grep -o 'dev.*' | awk '{ print $2 }'`
